...
diskNode *diskHead;
typedef struct fileTable //文件块结构体，即 FCB
{
    char fileName[10];
...
//UFD *headFile;
typedef struct master_file_directory //主文件目录 MFD
{
    char userName[10];
    char password[10];
    UFD *user;
...
int main() {
    ...
    while (1) {
        ...
        while (userID != -1) {
            printf("———————————————————————————————————————\n");
            printf(" create-创建 格式：create a1 1000 rw,将创建名为 a1,长度为 1000 字节可读可写的文件\n");
...
void fileCreate(char fileName[], int length, char fileKind[]) {
    ...
    if (requestDist(startPos, length)) {
        ...
        fileNode->file = (fileTableN *) malloc(
                sizeof(fileTableN)); //这一步必不可少，因为 fileNode 里面的指针也需要申请地址，否则 fileNode->file 指向会出错
...
void fileDir(char userName[])
{
    ...
    else
    {
        ...
        printf("***************************************************\n");
        printf("文件名  文件长度  文件在磁盘的起始地址  文件类型创建时间\n");
        for(;p!=NULL;p=p->next)
